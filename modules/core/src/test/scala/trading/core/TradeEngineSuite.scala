...
object TradeEngineSuite extends FunSuite with Checkers:
  ...
  test("Trade engine state machine") {
    val st1 =
      TradeEngine.fsm.runS(TradeState.empty, TradeCommand.Create(id, cid, s, TradeAction.Ask, p1, q1, "test", ts))
    val ex1 = TradeState(On, Map(s -> Prices(ask = Map(p1 -> q1), bid = Map.empty, p1, p1)))

    val st2 = TradeEngine.fsm.runS(st1, TradeCommand.Update(id, cid, s, TradeAction.Ask, p2, q2, "test", ts))
    val ex2 = TradeState(On, Map(s -> Prices(ask = Map(p1 -> q1, p2 -> q2), bid = Map.empty, p2, p1)))

    val st3 = TradeEngine.fsm.runS(st2, TradeCommand.Delete(id, cid, s, TradeAction.Ask, p1, "test", ts))
    val ex3 = TradeState(On, Map(s -> Prices(ask = Map(p2 -> q2), bid = Map.empty, p2, p1)))

    val st4 = TradeEngine.fsm.runS(st3, TradeCommand.Create(id, cid, s, TradeAction.Bid, p1, q1, "test", ts))
